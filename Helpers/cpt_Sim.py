...
class cpt_Sim(object):
    def __init__(self, data_dir, embedding_name, embedding_dim, tag_coefficient, k, b, weighted_intervals,
        ...
        self.wordindex2IDF = {}
        self.average_len = 0
        self.stopwords= new_Para.param.stop_words if remove_stopwords else set()
...
def get_sims_dict(process_new,remove_stopwords):
    ...
    cs = cpt_Sim(new_Para.param.data_dir, embedding_name, embedding_dim, tag_coefficient, k, b, weighted_intervals,
                 unweighted_intervals, process_new,remove_stopwords)
    return cs # 返回对象
